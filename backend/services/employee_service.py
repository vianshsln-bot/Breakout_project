...
class EmployeeService:
    ...
    def _select_employee(self, user_id: str) -> Dict[str, Any]:
        ...
        return getattr(resp, "data", {}) or {}  # minimal projection [web:21][web:39]
    # services/employee_service.py (relevant parts)
    def create_employee_with_role(self, payload: EmployeeCreate) -> EmployeeOut:
        try:
            ...
            raise DatabaseError("Authentication error") from e  # consider mapping specific codes later [web:15][web:12]

        user = getattr(signup, "user", None)
        if not user:
            raise DatabaseError("User not created")  # signup must return a user object [web:15]

        # Update trigger-created employee
        update_fields = {"email": str(payload.email), "role": payload.role}
        if payload.name is not None: update_fields["name"] = payload.name
        if payload.phone_number is not None: update_fields["phone_number"] = payload.phone_number
        if payload.branch_id is not None: update_fields["branch_id"] = payload.branch_id
        self.client.table(self.table_name).update(update_fields).eq("id", user.id).execute()  # narrow update [web:21]
        row = self._select_employee(user.id)  # single-row fetch [web:39]
        if not row:
